        ...
        App.getInstance().getAppDatabase().dialogRepository().getChatDialogById(dialogId)
                ...
                .subscribe(new DisposableMaybeObserver<List<Dialog>>() {
                    ...
                    public void onSuccess(@NotNull List<Dialog> dialogs) {
                        if (isMessageCreatedDialog(chatMessage) && !dialogs.isEmpty()) {
                    ...
                    public void onComplete() {
                        Log.i(TAG, "onComplete() method in onGlobalMessageReceived");
                    }
        ...
        if (chatMessage.isMarkable()) {
            App.getInstance().getAppDatabase().dialogRepository().getChatDialogById(dialogId)
                    ...
                    .subscribe(new DisposableMaybeObserver<List<Dialog>>() {
                        ...
                        public void onSuccess(@NotNull List<Dialog> dialogs) {
                            if (!dialogs.isEmpty()) {
                                DialogRepositoryHelper.getInstance().updateDialog(dialogId, chatMessage);
                                notifyListenersDialogUpdated(dialogId);
                            } else {
                                ChatHelper.getInstance().getDialogById(dialogId, new QBEntityCallback<QBChatDialog>() {
                                    ...
                                    public void onSuccess(QBChatDialog qbChatDialog, Bundle bundle) {
                                        ...
                                        App.getInstance().getAppDatabase().dialogRepository().putDialog(DatabaseConvertor.convert(qbChatDialog))
                                                .subscribeOn(Schedulers.io())
                                                .observeOn(AndroidSchedulers.mainThread())
