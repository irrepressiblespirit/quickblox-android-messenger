...
public class ValidationUtils {
    ...
    private static boolean isEnteredTextValid(Context context, EditText editText, int resFieldName, int maxLength, boolean checkLogin) {
        ...
        Pattern patternPassword = Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{" + (maxLength - 1) + ",}+$");
        if (checkLogin) {
            ...
            Matcher matcherUserName = patternPassword.matcher(editText.getText().toString().trim());
    ...
    public static boolean isPasswordValid(Context context, EditText editText) {
