...
import com.quickblox.sample.chat.java.repository.helpers.DialogRepositoryHelper;
...
public class DialogsActivity extends BaseActivity implements DialogsManager.ManagingDialogsCallbacks {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getSupportActionBar() != null) {
            getSupportActionBar().setTitle(getString(R.string.dialogs_logged_in_as, currentUser.getLogin()));
        }
        initUi();
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case REQUEST_SELECT_PEOPLE:
                    ...
                    isPrivateDialogExist(selectedUsers).thenAccept(privateDialogIsExist -> {
                        String chatName = data.getStringExtra(SelectUsersActivity.EXTRA_CHAT_NAME);
                        if (privateDialogIsExist) {
                            selectedUsers.remove(ChatHelper.getCurrentUser());
                            isProcessingResultInProgress = false;
                            DialogRepositoryHelper.getInstance().getPrivateDialogWithUser(selectedUsers.get(0))
                                    .thenAccept(result -> {
                                        if (result != null) {
                                            QBChatDialog existingPrivateDialog = DatabaseConvertor.convert(result);
    ...
    private CompletableFuture<Boolean> isPrivateDialogExist(ArrayList<QBUser> allSelectedUsers) {
        ArrayList<QBUser> selectedUsers = new ArrayList<>();
        selectedUsers.addAll(allSelectedUsers);
        selectedUsers.remove(ChatHelper.getCurrentUser());
        if (selectedUsers.size() == 1) {
            return DialogRepositoryHelper.getInstance().hasPrivateDialogWithUser(selectedUsers.get(0));
        }
    ...
    private void loadUpdatedDialog(String dialogId) {
        ChatHelper.getInstance().getDialogById(dialogId, new QbEntityCallbackImpl<QBChatDialog>() {
            ...
            public void onSuccess(QBChatDialog result, Bundle bundle) {
                App.getInstance().getAppDatabase().dialogRepository().putDialog(DatabaseConvertor.convert(result))
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(new DisposableSingleObserver<Long>() {
                            @Override
                            public void onSuccess(@NotNull Long number) {
    ...
    private void userLogout() {
        ...
        QBUsers.signOut().performAsync(new QBEntityCallback<Void>() {
            ...
            public void onSuccess(Void aVoid, Bundle bundle) {
                ...
                App.getInstance().getAppDatabase().dialogRepository().clear()
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(new DisposableSingleObserver<Integer>() {
                            @Override
                            public void onSuccess(@NotNull Integer integer) {
                                Log.i(TAG, "operation clear() is success");
                            }
    ...
    private void initUi() {
        ...
        App.getInstance().getAppDatabase().dialogRepository().getDialogs()
                ...
                .subscribe(new DisposableMaybeObserver<List<Dialog>>() {
                    ...
                    public void onSuccess(@NotNull List<Dialog> dialogs) {
                        if (!dialogs.isEmpty()) {
                            ArrayList<QBChatDialog> chatDialogs = new ArrayList<>(DatabaseConvertor.convertToChatDialogs(dialogs));
                            dialogsAdapter = new DialogsAdapter(DialogsActivity.this, chatDialogs);
                            dialogsListView.setEmptyView(emptyHintLayout);
                            dialogsListView.setAdapter(dialogsAdapter);
                        } else {
                            dialogsAdapter = new DialogsAdapter(DialogsActivity.this, null);
                            dialogsListView.setEmptyView(emptyHintLayout);
                            dialogsListView.setAdapter(dialogsAdapter);
                        }
                    }
    ...
    private void createDialog(final ArrayList<QBUser> selectedUsers, String chatName) {
        ...
        QbDialogUtils.getQbUsersAndLocallySaveByIds(ids).thenAccept(result -> {
    ...
    private void loadDialogsFromQb(final boolean silentUpdate, final boolean clearDialogHolder) {
        ...
        App.getInstance().getAppDatabase().dialogRepository().getDialogs()
                ...
                .subscribe(new DisposableMaybeObserver<List<Dialog>>() {
                    ...
                    public void onSuccess(@NotNull List<Dialog> dialogs) {
                        if (!dialogs.isEmpty()) {
                            requestBuilder.setSkip(clearDialogHolder ? 0 : dialogs.size());
                        }
                    }
                    ...
                    @Override
                    public void onComplete() {
                        Log.i(TAG, "onComplete() method in loadDialogsFromQb");
                    }
                });
        ...
        private void deletePrivateDialogs(final List<QBChatDialog> privateDialogsToDelete) {
            ChatHelper.getInstance().deletePrivateDialogs(privateDialogsToDelete, new QBEntityCallback<ArrayList<String>>() {
                ...
                public void onSuccess(ArrayList<String> dialogsIds, Bundle bundle) {
                    ...
                    App.getInstance().getAppDatabase().dialogRepository().deleteDialogsByIds(dialogsIds)
                            .subscribeOn(Schedulers.io())
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribe(new DisposableSingleObserver<Integer>() {
                                @Override
        ...
        private void leaveGroupDialogs(final List<QBChatDialog> groupDialogsToDelete) {
            ChatHelper.getInstance().leaveGroupDialogs(groupDialogsToDelete, new QBEntityCallback<List<QBChatDialog>>() {
                ...
                public void onSuccess(List<QBChatDialog> qbChatDialogs, Bundle bundle) {
                    ...
                    App.getInstance().getAppDatabase().dialogRepository().deleteDialogs(DatabaseConvertor.convertToDialogList(qbChatDialogs).toArray(new Dialog[qbChatDialogs.size()]))
                            .subscribeOn(Schedulers.io())
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribe(new DisposableSingleObserver<Integer>() {
                                @Override
