...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.NewMessageViewHolder> implements StickyRecyclerHeadersAdapter<RecyclerView.ViewHolder> {
    ...
    private void onBindViewMsgLeftHolder(NewMessageViewHolder holder, QBChatMessage chatMessage) {
        getSenderName(chatMessage).thenAccept(senderName -> holder.tvUserName.setText(senderName));
    ...
    private void fillAvatarHolder(final NewMessageViewHolder holder, QBChatMessage chatMessage) {
        ...
        App.getInstance().getAppDatabase().userRepository().getUserById(chatMessage.getSenderId())
                ...
                .subscribe(new DisposableMaybeObserver<List<User>>() {
                    ...
                    public void onSuccess(@NotNull List<User> users) {
                        if (!users.isEmpty()) {
                            ...
                            String avatarTitle = String.valueOf(user.getLogin().charAt(0));
                    ...
                    public void onComplete() {
                        Log.i(TAG, "onComplete() method in fillAvatarHolder");
                    }
                });
    }
    ...
    private CompletableFuture<String> getSenderName(QBChatMessage chatMessage) {
                ...
                .subscribe(new DisposableMaybeObserver<List<User>>() {
                    ...
                    public void onSuccess(@NotNull List<User> users) {
                        if (!users.isEmpty()) {
                            ...
                            if (sender != null && !TextUtils.isEmpty(sender.getLogin())) {
                                future.complete(sender.getLogin());
                            }
                        }
                        if (!future.isDone()) {
                            future.complete("unknown_sender_name");
                        }
                    }
